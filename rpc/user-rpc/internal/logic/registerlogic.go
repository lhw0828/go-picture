...
func (l *RegisterLogic) Register(in *user.RegisterRequest) (*user.RegisterResponse, error) {
	...
	if in.UserPassword != in.CheckPassword {
		return nil, constants.NewCodeError(constants.PasswordNotMatch, "两次输入的密码不一致")
	}
	...
	if existUser != nil {
		return nil, constants.NewCodeError(constants.UserExist, "账号已存在")
	}
	...
	if err != nil {
		...
		return nil, constants.NewCodeError(constants.RegisterFail, "注册失败")
	}
